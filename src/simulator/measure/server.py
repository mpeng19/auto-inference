"""Server-side measurement, scraped from SGLang's Prometheus endpoint, and the
server's lifecycle around a sweep.

The price is read from here: `forward_execution_seconds_total` split by phase
is GPU time from the device timer, and the token counters are the
denominators. The SLO is judged on client-side latency instead, because the
server's histograms are bucketed too coarsely for a percentile bound; the
server's own TTFT is still recorded so the client-plus-network overhead can be
seen when a number looks wrong.
"""
...
class BatchSampler:
    ...
    The running batch is what a level's GPU-seconds have to be read against:
    a level nominally at 12 users that the scheduler ran at a batch of 3 is
    a different measurement. Before/after gauge scrapes cannot tell those
    apart -- they catch the queue at two instants, not what the scheduler
    actually ran.
...
async def wait_until_ready(base_url: str, timeout_s: float = 1800.0,
    ...
    Model load is minutes, not seconds; benchmarking a still-loading server
    is a classic way to produce garbage TTFT numbers.
...
async def complete(base_url: str, model: str, prompt: str, max_tokens: int,
    ...
    """Non-streaming completion, for canaries and quality suites, where only
    the text matters."""
